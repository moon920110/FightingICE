 ...
 */
public class Node11 extends Node{
     ...
     * Parent node
     */
    private Node11 parent;

    /**
     * Child node
     */
    private Node11[] children;
    ...
    public Node11(FrameData frameData, Node11 parent, LinkedList<Action> myActions,
...
//        Map<FieldName, Object> arguments =
    ...
    public double uctSingle() {
        ...
        Node11 selectedNode = null;
        double bestUcb;

        bestUcb = -99999;

        for (Node11 child : this.children) {
            if (child.games == 0) {
    ...
    public void createNode() {
        ...
        this.children = new Node11[myActions.size()];
        ...
        for (int i = 0; i < children.length; i++) {
            ...
            children[i] =
                    new Node11(frameData, this, myActions, oppActions, gameData,
    ...
    public Action getBestScoreAction() {
        ...
        for (int i = 0; i < children.length; i++) {
            ...
            System.out.println("[Node11]" + "Evaluation value:" + children[i].score / children[i].games + ",Number of trials:"
                    + children[i].games + ",ucb:" + children[i].ucb + ",Action:" + myActions.get(i));
...
//                : -(Double) valenceResult.get(FieldName.create("probability(0)"));
        ...
        return -challengeScore + competenceScore;
...
//                .getCharacter(false).getHp() - myOriginalHp) - (fd.getCharacter(true).getHp() - oppOriginalHp);
    ...
    public void printNode(Node11 node) {
    ...
    public double runWorkers(Node11 selectedNode, ExecutorService executorService) throws InterruptedException {
        double score = 0;
        CountDownLatch countDownLatch = new CountDownLatch(selectedNode.parent.children.length);

        for (Node11 child : selectedNode.parent.children) {
