...
public class Node4 extends Node{
    ...
    public Node4(FrameData frameData, Node4 parent, LinkedList<Action> myActions,
...
//        Map<FieldName, Object> arguments =
    ...
    public double uctSingle() {
        ...
        Node4 selectedNode = null;
        double bestUcb;

        bestUcb = -99999;

        for (Node4 child : this.children) {
            if (child.games == 0) {
    ...
    public void createNode() {
        ...
        this.children = new Node4[myActions.size()];
        ...
        for (int i = 0; i < children.length; i++) {
            ...
            children[i] =
                    new Node4(frameData, this, myActions, oppActions, gameData,
    ...
    public Action getBestScoreAction() {
        ...
        for (int i = 0; i < children.length; i++) {
            ...
            System.out.println("[Node4]" + "Evaluation value:" + children[i].score / children[i].games + ",Number of trials:"
                    + children[i].games + ",ucb:" + children[i].ucb + ",Action:" + myActions.get(i));
...
//                .getCharacter(false).getHp() - myOriginalHp) - (fd.getCharacter(true).getHp() - oppOriginalHp);
    ...
    public void printNode(Node4 node) {
    ...
    public double runWorkers(Node4 selectedNode, ExecutorService executorService) throws InterruptedException {
        double score = 0;
        CountDownLatch countDownLatch = new CountDownLatch(selectedNode.parent.children.length);

        for (Node4 child : selectedNode.parent.children) {
