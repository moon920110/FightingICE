...
public class Node3 extends Node{
    ...
    public Node3(FrameData frameData, Node3 parent, LinkedList<Action> myActions,
...
//        Map<FieldName, Object> arguments =
    ...
    public double uctSingle() {
        ...
        Node3 selectedNode = null;
        double bestUcb;

        bestUcb = -99999;

        for (Node3 child : this.children) {
            if (child.games == 0) {
    ...
    public void createNode() {
        ...
        this.children = new Node3[myActions.size()];
        ...
        for (int i = 0; i < children.length; i++) {
            ...
            children[i] =
                    new Node3(frameData, this, myActions, oppActions, gameData,
    ...
    public Action getBestScoreAction() {
        ...
        for (int i = 0; i < children.length; i++) {
            ...
            System.out.println("[Node3]" + "Evaluation value:" + children[i].score / children[i].games + ",Number of trials:"
                    + children[i].games + ",ucb:" + children[i].ucb + ",Action:" + myActions.get(i));
...
//                .getCharacter(false).getHp() - myOriginalHp) - (fd.getCharacter(true).getHp() - oppOriginalHp);
    ...
    public void printNode(Node3 node) {
    ...
    public double runWorkers(Node3 selectedNode, ExecutorService executorService) throws InterruptedException {
        double score = 0;
        CountDownLatch countDownLatch = new CountDownLatch(selectedNode.parent.children.length);

        for (Node3 child : selectedNode.parent.children) {
