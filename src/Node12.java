 ...
 */
public class Node12 extends Node{
     ...
     * Parent node
     */
    private Node12 parent;

    /**
     * Child node
     */
    private Node12[] children;
    ...
    public Node12(FrameData frameData, Node12 parent, LinkedList<Action> myActions,
...
//        Map<FieldName, Object> arguments =
    ...
    public double uctSingle() {
        ...
        Node12 selectedNode = null;
        double bestUcb;

        bestUcb = -99999;

        for (Node12 child : this.children) {
            if (child.games == 0) {
    ...
    public void createNode() {
        ...
        this.children = new Node12[myActions.size()];
        ...
        for (int i = 0; i < children.length; i++) {
            ...
            children[i] =
                    new Node12(frameData, this, myActions, oppActions, gameData,
    ...
    public Action getBestScoreAction() {
        ...
        for (int i = 0; i < children.length; i++) {
            ...
            System.out.println("[Node12]" + "Evaluation value:" + children[i].score / children[i].games + ",Number of trials:"
                    + children[i].games + ",ucb:" + children[i].ucb + ",Action:" + myActions.get(i));
...
//                : -(Double) competenceResult.get(FieldName.create("probability(0)"));
        ...
        return immersionScore + valenceScore;
...
//                .getCharacter(false).getHp() - myOriginalHp) - (fd.getCharacter(true).getHp() - oppOriginalHp);
    ...
    public void printNode(Node12 node) {
    ...
    public double runWorkers(Node12 selectedNode, ExecutorService executorService) throws InterruptedException {
        double score = 0;
        CountDownLatch countDownLatch = new CountDownLatch(selectedNode.parent.children.length);

        for (Node12 child : selectedNode.parent.children) {
