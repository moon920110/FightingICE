 ...
 */
public class Node9 extends Node{
     ...
     * Parent node
     */
    private Node9 parent;

    /**
     * Child node
     */
    private Node9[] children;
    ...
    public Node9(FrameData frameData, Node9 parent, LinkedList<Action> myActions,
...
//        Map<FieldName, Object> arguments =
    ...
    public double uctSingle() {
        ...
        Node9 selectedNode = null;
        double bestUcb;

        bestUcb = -99999;

        for (Node9 child : this.children) {
            if (child.games == 0) {
    ...
    public void createNode() {
        ...
        this.children = new Node9[myActions.size()];
        ...
        for (int i = 0; i < children.length; i++) {
            ...
            children[i] =
                    new Node9(frameData, this, myActions, oppActions, gameData,
    ...
    public Action getBestScoreAction() {
        ...
        for (int i = 0; i < children.length; i++) {
            ...
            System.out.println("[Node9]" + "Evaluation value:" + children[i].score / children[i].games + ",Number of trials:"
                    + children[i].games + ",ucb:" + children[i].ucb + ",Action:" + myActions.get(i));
...
//                .getCharacter(false).getHp() - myOriginalHp) - (fd.getCharacter(true).getHp() - oppOriginalHp);
    ...
    public void printNode(Node9 node) {
    ...
    public double runWorkers(Node9 selectedNode, ExecutorService executorService) throws InterruptedException {
        double score = 0;
        CountDownLatch countDownLatch = new CountDownLatch(selectedNode.parent.children.length);

        for (Node9 child : selectedNode.parent.children) {
